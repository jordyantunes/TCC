  ...
  int step)
{
     ...
     of the two eigenvalues of the Z matrix */
  {
    ...
    int displace = tc->window_width + 5;
    ...
    if (step < 1)
       step = tc->window_width;
	
    /* For most of the pixels in the image, do ... */
    ptr = pointlist;
    for (y = bordery + displace; y < (nrows - bordery) - displace ; y += (tc->nSkippedPixels + step)) {
      for (x = borderx + displace; x < (ncols - borderx) - displace ; x += (tc->nSkippedPixels + step))  {
  ...
  /* Sort the features  */
  /*Alterei: comentei a ordenação para manter a ordem das característica*/
  ...
  int step)
{
  ...
  if (nLostFeatures > 0)
    _KLTSelectGoodFeatures(tc, img, ncols, nrows, 
                           fl, REPLACING_SOME, weights, step);
